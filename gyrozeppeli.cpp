...
Gyro::Gyro() {
    ...
    setPos(700, 300);     // Posición inicial fija en la derecha
}

// Cargo los 2 frames de ataque de Gyro desde el sprite sheet.
// Para ajustar posiciones solo toca las constantes arriba, no este código.
// Los sprites del sheet apuntan a la DERECHA, pero Gyro siempre lanza
...
void Gyro::cargarFramesAtaque()
{
    QPixmap sprites(":/sprites_juego.png");


    struct { int x, ancho; } data[] = {
        {X_FRAME0_ATAQUE, ANCHO_FRAME0},  // frame 0: Gyro apuntando con el dedo
        {X_FRAME1_ATAQUE, ANCHO_FRAME1},  // frame 1: Gyro con la bola ya lanzada
    };
...
QPixmap Gyro::quitarFondo(const QPixmap &original)
{
...
QPixmap Gyro::reflejarHorizontal(const QPixmap &original)
{
    QTransform espejo;
    espejo.scale(-1, 1);
...
void Gyro::actualizarComportamiento(int tiempoRestante, qreal jotaroY, bool camaraLenta) {
    ...
    if (tiempoRestante > 40) {
        ...
        estadoActual = Lluvia; // ¡Últimos 15 segundos!
    }

    // --- EJECUCIÓN DE ATAQUES SEGÚN EL ESTADO ---
    if (estadoActual == Tranquilo) {
        // Cada 50 frames se teletransporta e intenta apuntar cerca a Jotaro
        if (contadorAtaque >= 50) {
            contadorAtaque = 0;

            int posicionesY[3] = {150, 300, 450};
            setPos(700, posicionesY[std::rand() % 3]);
        ...
        // Se tepea rápido y dispara ráfagas triples
        if (contadorAtaque >= 45) {
            contadorAtaque = 0;

            setPos(700, (std::rand() % 300) + 150);
        ...
        if (contadorAtaque >= 38) {
            ...
            // Segunda bola ocasional para que haya ráfagas sin que sea un caos constante
            if (std::rand() % 10 < 4) {
...
void Gyro::dispararBola(SteelBall::TipoBola tipo, SteelBall::TipoTrayectoria trayec, int offsetY) {
...
QList<SteelBall*> Gyro::tomarNuevasBolas() {
